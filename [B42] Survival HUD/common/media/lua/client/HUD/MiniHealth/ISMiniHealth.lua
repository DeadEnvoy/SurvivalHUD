require "MiniHealth/ISMiniHealth";

if not ISMiniHealth then
	return;
end
...
function ISMiniHealth:checkNewResolution()
	self:setX(OVERRIDE_X);
	self:setY(getCore():getScreenHeight() - self.height - OVERRIDE_Y);
end
...
function ISMiniHealth:isMouseOver()
	...
	if not self.javaObject:isMouseOver() then
		return false;
	end
...
function ISMiniHealth:getDamagedParts()
	...
	for i = 1, bodyParts:size() do
		...
		if bodyPart:HasInjury() or bodyPart:bandaged() or bodyPart:stitched() or bodyPart:getSplintFactor() > 0 or bodyPart:getAdditionalPain() > 10 or bodyPart:getStiffness() > 5 or (ISHealthPanel.cheat and bodyPart:getStiffness() > 0) then
...
function ISMiniHealth:getBodyPartStatusText(bodyPart)
	...
	if bodyPart:getPlantainFactor() > 0 then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - factor " .. round(bodyPart:getPlantainFactor(), 2) .. "\n";
		end
	end
	if bodyPart:getComfreyFactor() > 0 then
		injuryText = injuryText .. "<RGB:0.28,0.89,0.28>- " .. getText("ContextMenu_ComfreyCataplasm") .. "\n";
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - factor " .. round(bodyPart:getComfreyFactor(), 2) .. "\n";
		end
	end
	if bodyPart:getGarlicFactor() > 0 then
		injuryText = injuryText .. "<RGB:0.28,0.89,0.28>- " .. getText("ContextMenu_GarlicCataplasm") .. "\n";
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - factor " .. round(bodyPart:getGarlicFactor(), 2) .. "\n";
		end
	end
	...
	if bodyPart:scratched() then
		...
		if doctorLevel > 2 or cheat then
			if bodyPart:getScratchTime() > 17 then
				extraText = " (" .. getText("IGUI_health_Severe") .. ")";
			elseif bodyPart:getScratchTime() > 14 then
				extraText = " (" .. getText("IGUI_health_Moderate") .. ")";
			end
		end
		injuryText = injuryText .. "<RGB:0.89,0.28,0.28>- " .. getText("IGUI_health_Scratched") .. extraText .. "\n";
		if cheat and bodyPart:getScratchTime() > 0 then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - scratch Time " .. round(bodyPart:getScratchTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:isCut() then
		...
		if cheat and bodyPart:getCutTime() > 0 then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - cut Time " .. round(bodyPart:getCutTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:deepWounded() then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - deepWound Time " .. round(bodyPart:getDeepWoundTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:bitten() then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - bite Time " .. round(bodyPart:getBiteTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:getAdditionalPain() > 10 then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - additional pain " .. round(bodyPart:getAdditionalPain(), 2) .. "\n";
		end
	end
	...
	if bodyPart:getStiffness() >= 5 then
		showStiffness = true;
	elseif ISHealthPanel.cheat and bodyPart:getStiffness() > 0 then
		showDebugStiffness = true;
	end
	if showStiffness and bodyPart:getStiffness() < 20 then
		minorStiffness = true;
	end
	...
	if minorStiffness then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:1,0.58,0>- " .. getText("(Debug - HAS NO EFFECT ON THE PLAYER!)") .. round(bodyPart:getStiffness(), 2) .. "/100\n";
		end
	elseif showStiffness then
		injuryText = injuryText .. "<RGB:0.89,0.28,0.28>- " .. getText("IGUI_health_Stiffness") .. "\n";
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28>- " .. getText("(Debug) ") .. round(bodyPart:getStiffness(), 2) .. "/100\n";
		end
	...
	if bodyPart:bleeding() then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - bleeding Time " .. round(bodyPart:getBleedingTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:getFractureTime() > 0 and bodyPart:getSplintFactor() == 0 then
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - fracture Time " .. round(bodyPart:getFractureTime(), 2) .. "\n";
		end
	end
	...
	if bodyPart:getSplintFactor() > 0 then
		...
		if doctorLevel > 4 or cheat then
			if bodyPart:getSplintFactor() > 4 then
				extraText = " (" .. getText("IGUI_health_Good") .. ")";
		...
		if cheat then
			injuryText = injuryText .. "<RGB:0.89,0.28,0.28> - splint Factor " .. round(bodyPart:getSplintFactor(), 2) .. "\n";
		end
	end
	...
	if bodyPart:isInfectedWound() and not bodyPart:bandaged() then
		if doctorLevel > 8 or (bodyPart:getWoundInfectionLevel() * 10 >= (2.5 - doctorLevel)) or cheat then
			...
			if cheat then
				injuryText = injuryText .. "<RGB:1,0.28,0> - WOUND infection Time " .. round(bodyPart:getWoundInfectionLevel(), 4) .. "\n";
			end
		end
	end
	...
	if bodyPart:stitched() then
		...
		if doctorLevel > 6 or cheat then
			if bodyPart:getStitchTime() > 40 then
				...
				extraText = " (" .. getText("IGUI_health_NeedTime") .. ")";
			end
		end
...
function ISMiniHealth:renderTooltip()
	...
	local description = "<RGB:1,1,1>" .. getText("IGUI_health_Overall_Body_Status") .. "\n";
	local health = self.player:getBodyDamage():getHealth();
	...
	local r, g, b = 1.0, 1.0 - InjuryRedTextTint, 1.0 - InjuryRedTextTint;
