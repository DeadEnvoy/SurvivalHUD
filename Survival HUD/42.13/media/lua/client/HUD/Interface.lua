...
local function getGameVersion()
    return tonumber(getCore():getVersionNumber():match("^(%d+)"))
end

local MOODLE_NAMES = (getGameVersion() >= 42)
and {
...
function IconHungerThirstUI:calculateDimensions()
    ...
    if not stats or not body then return self.dimensions or
        { groupWidths = {}, totalWidth = 0, totalHeight = CONFIG.iconSize } end

    local infectionLevel = body:getApparentInfectionLevel()
    local sicknessLevel = stats:get(CharacterStat.SICKNESS)
...
function IconHungerThirstUI:render()
    ...
    local values = {
        ...
        sickness = math.max(0, math.min(100, (stats:get(CharacterStat.SICKNESS) / 1) * 100)),
